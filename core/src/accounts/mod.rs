...
use crescent_auth::microsoft::{DEFAULT_CLIENT_ID, begin_device_flow, poll_for_token};
...
impl MinecraftAccount {
    ...
    pub async fn microsoft(_email: String) -> Result<Self, BotError> {
        Err(BotError::ConnectionError {
            message: "Direct Microsoft login is deprecated. Please use MicrosoftAuthFlow instead."
                .to_string(),
        })
    }
}
...
impl MicrosoftAuthFlow {
    ...
    pub async fn initiate(&self) -> Result<MicrosoftAuthInfo, BotError> {
        let response = begin_device_flow(&self.client, DEFAULT_CLIENT_ID)
            .await
    ...
    pub async fn poll(&self, info: MicrosoftAuthInfo) -> Result<MinecraftAccount, BotError> {
        ...
        let msa = poll_for_token(&self.client, DEFAULT_CLIENT_ID, &dev_resp)
            .await
            .map_err(|e| BotError::ConnectionError {
                message: e.to_string(),
            })?;

        let (mc_token, profile) = authenticate(&self.client, &msa.access_token)
            .await
            .map_err(|e| BotError::ConnectionError {
                message: e.to_string(),
            })?;
