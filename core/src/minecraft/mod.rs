...
impl MinecraftBot {
    ...
    pub async fn connect(
        &self,
        account: &crate::accounts::MinecraftAccount,
        address: String,
        protocol_version: i32,
    ) -> Result<(), BotError> {
        self.connect_core(account.account.clone(), address, protocol_version)
            .await
    }
    ...
    pub async fn listen(
        &self,
        callback: napi::threadsafe_function::ThreadsafeFunction<BotEvent>,
    ) -> Result<(), BotError> {
...
impl MinecraftBot {
    pub async fn connect(
        &self,
        account: Arc<crate::accounts::MinecraftAccount>,
        address: String,
        protocol_version: i32,
    ) -> Result<(), BotError> {
        self.connect_core(account.account.clone(), address, protocol_version)
...
impl MinecraftBot {
    async fn connect_core(
        &self,
        account: crescent_protocol::ClientAccount,
        address: String,
        protocol_version: i32,
    ) -> Result<(), BotError> {
        ...
        let client = ProtocolClient::connect(
            ...
            port,
            account,
            ProtocolVersion::from_raw(protocol_version),
        )
        .await
        .map_err(|e| BotError::ConnectionError {
            message: e.to_string(),
        })?;
    ...
    async fn listen_core(&self, listener: Arc<dyn BotEventListener>) -> Result<(), BotError> {
        let mut rx = {
            ...
            if let Some(ref client) = *guard {
                ...
                return Err(BotError::ConnectionError {
...
#[cfg_attr(feature = "node-binding", napi_derive::napi)]
#[cfg_attr(feature = "native-binding", uniffi::export)]
pub fn is_protocol_supported(protocol_version: i32) -> bool {
    crescent_protocol::ProtocolVersion::from_raw(protocol_version).is_supported()
}

#[cfg_attr(feature = "node-binding", napi_derive::napi)]
#[cfg_attr(feature = "native-binding", uniffi::export)]
pub fn get_supported_versions() -> Vec<i32> {
    crescent_protocol::get_supported_versions()
}
